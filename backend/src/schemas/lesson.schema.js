...
//
// The model produces search intent, never a URL — it cannot know which videos
// exist, and a hallucinated video id renders as a broken embed. Resolution to an
// actual video happens later, asynchronously, in the video worker.
export const videoBlockSchema = z.object({
  type: z.literal('video'),
  query: z.string().trim().min(1, 'video query cannot be empty').max(200),
  caption: z.string().trim().max(300).optional(),
});

// NOTE: the fallback query is deliberately NOT generated by the model.
//
// Asking for one made generation fail roughly two runs in three: the model would
// degenerate on the field, emitting hundreds of repeated words — and in one run,
// padding text explaining that it was padding — until it blew the output limit
// and returned truncated JSON. Gemini's structured output treats `maxLength` as
// advisory, so the schema could not stop it.
//
// A fallback is "the same query with qualifiers removed", which is a mechanical
// string transformation. deriveFallbackQuery in services/video/query.js does it
// deterministically, for free, and cannot fail.
